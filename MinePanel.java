...
/**
 * <b>Title:</b> Project5<br>
 * <b>Filename:</b> MinePanel.java<br>
 * <b>Date Written:</b> December 20, 2024<br>
 * <b>Due Date:</b> December 20, 2024<br>
 * <p>
 * Description: MindSweeper panel
 * 
 * @author @Ricky_Cui N00967649
 * @version 1.0
 */
class MinePanel extends JPanel implements MouseListener {
	...
	/**
	 * displayRight called when right mouse button is clicked right mouse click on a
	 * button adds or removes a flag
	 *
	 * @param g  Graphics Context
	...
	public void displayRight(Graphics g, int cx, int cy) {
		...
		{
			g.fill3DRect(button[cy / 20][cx / 20].getNumber() % 16 * 20, button[cy / 20][cx / 20].getNumber() / 16 * 20,
					18, 18, true);
	...
	/**
	 * displayLeftt called when left mouse button is clicked left mouse click on a
	 * button, exposes number of mines in the area or exposes safe buttons connected
	 * to this button, if this button is a safe button
	 *
	 * @param g  Graphics Context
	 * @param cx x coordinate of the Panel
	 * @param cy y coordinate of the Panel
		...
		try {
			...
			if (button[y][x].getState() == 0) {
				...
				if (button[y][x].getNoOfMinesAround() == 0) // no mines around
					...
					removeAdjacentButtons(g, button[y][x].getNumber() / cols, button[y][x].getNumber() % rows);
				...
				else // if there are mines nearby
				{
					...
					if (button[y][x].isMined()) // bomb here
					{
						...
						gameOver = true;
					} else if (button[y][x].getState() == 0) // no flag here
					{
						button[y][x].showIt(g);
					}
				}
...
//		System.out.println("x = " + x + " y = "+y);
	...
	public void removeAdjacentButtons(Graphics g, int r, int c) {
		...
		if (r < 0 || c < 0 || r >= button.length || c >= button[0].length) {
			return;
		}

		// If the button is already showing, stop recursion
		if (button[r][c].isShowing()) {
			return;
		}
		...
		// If the current button has mines around it, stop recursion
		if (button[r][c].getNoOfMinesAround() > 0) {
			return;
		}

		removeAdjacentButtons(g, r - 1, c); // Up
		removeAdjacentButtons(g, r + 1, c); // Down
		removeAdjacentButtons(g, r, c - 1); // Left
		removeAdjacentButtons(g, r, c + 1); // Right
		removeAdjacentButtons(g, r - 1, c - 1); // Top-left
		removeAdjacentButtons(g, r - 1, c + 1); // Top-right
		removeAdjacentButtons(g, r + 1, c - 1); // Bottom-left
		removeAdjacentButtons(g, r + 1, c + 1); // Bottom-right
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		if (!gameOver) {
			...
			} else if (e.getModifiers() == MouseEvent.BUTTON1_MASK) // left mouse button
			{
				displayLeft(g, cx, cy);
				if (gameOver) {
